...
%%
%% Turing grid lengths (1/21 of page dimensions)
\newlength{\turingcolwd}
\newlength{\turingcolht}
...
%%
%% Report metadata
\newcommand{\reportNumber}{10}
\newcommand{\reportTitle}{The Alan Turing Institute: Carbon emissions from computing}
\newcommand{\reportAuthors}{Rosie~Wood}
\newcommand{\reportAcknowledgment}{}
\newcommand{\reportPublicationDate}{March 2026}
\newcommand{\reportCopyrightDate}{2026}
\newcommand{\reportDOI}{10.5281/zenodo.19237594}
% \hypersetup{
%   colorlinks   = true, 
%   urlcolor     = darkblue,
%   linkcolor    = darkblue,
...
\usepackage{booktabs}
\usepackage{siunitx}
\sisetup{text-series-to-math=true} % To allow bold numbers in the table (but apparently not in Euler maths font...)
\usepackage{multirow}
...
%% --- Cover pages (scoped) ---
\begingroup
  \newgeometry{a4paper, left=\turingcolwd, vmargin=\turingcolht, textwidth=13\turingcolwd}
  ...
  {\includegraphics[width=5\turingcolwd, trim=6pt 0 0 5pt]{ATI_logo_white.pdf}};
  \end{tikzpicture}
  %%
  \vspace*{4\turingcolht}
  ...
  \color{white}%
  \newlength{\ttrwidth}%
  \settowidth{\ttrwidth}{\mbox{Technical Report}}%
  {\offinterlineskip\rule{\ttrwidth}{0.45ex}\\[0.5ex]
   Technical Report\par}
  \vspace{1\turingcolht}
  %%
  \mdseries\fontsize{21}{24}\selectfont
  \reportTitle\par
  \bigskip
  \mdseries\fontsize{13}{15}\selectfont
  \reportAuthors\par
  \medskip
  \reportPublicationDate\par
  \vspace{1\turingcolht}
  %%
  \mdseries
  \newlength{\ttrnumwidth}%
  \settowidth{\ttrnumwidth}{\mbox{Report number \reportNumber}}%
  {\offinterlineskip\rule{\ttrnumwidth}{0.15ex}\\[0.5ex]
  ...
  \vspace*{5\turingcolht}
  %%
  \fontsize{8}{9.2}\selectfont\raggedright%
...
In 2024, data centres consumed around \qty{1.5}{\percent} of the world’s total electricity and accounted for around \mtcooe{180} (\qty{0.3}{\percent} of global greenhouse gas emissions)~\cite{iea_energy_ai,unep_egr_2025}. Electricity consumption by data centres is forecast to more than double by 2030, largely driven by AI~\cite{iea_energy_ai}.

In this work, we estimate the greenhouse gas emissions from the Turing's use of its main compute resources (Microsoft Azure, Baskerville High Performance Computing system (HPC) and GitHub Actions) and identify areas in which we could reduce these emissions. Our results are summarised in Table~\ref{tab:summary_table}.

\begin{table}[ht]
  \centering
\begin{threeparttable}
  \caption{Summary of operational and embodied emissions from Azure, Baskerville and GitHub Actions between September 2024--August 2025.\label{tab:summary_table}}
  \sisetup{table-align-text-after=false}
  \begin{tabular}{@{}lrSS>{}S@{}}\toprule
    & & \multicolumn{3}{c}{{Emissions ${} / \, \unit[qualifier-mode=combine]{\tonne\of{\cooe}}$}} \\
    \cmidrule(l){3-5}
    \multirow{-2}{*}{Compute resource}  & \multirow{-2}{*}{\shortstack[r]{Electricity \\ ${}/\, \unit{\KWH}$}} & {Operational} & {Embodied} & {Total} \\ \midrule
    Microsoft Azure & {---}
      & 0\textsuperscript{\textdagger}  & 2.9 &  2.9 \\
    Baskerville (CHP) & \num{83300}
      &  25          & 6 &  31  \\
    GitHub Actions & \num{380}\tnote{\textdaggerdbl}
      &   0\textsuperscript{\textdagger} & 0.1 &  0.1 \\ \bottomrule
  \end{tabular}
  \begin{tablenotes}\small
    \item[\textdagger] Market-based operational emissions; location-based emissions cannot be calculated (see Section~\ref{sec:azure}).
    \item[\textdaggerdbl] Extrapolated from six months of data (March--August 2025).
 \end{tablenotes}
\end{threeparttable}
\end{table}
...
To help put these emissions from computing into perspective it can be helpful to compare against carbon emissions from other sources on similar scales:
 ...
 \begin{itemize}
     ...
     \item A flight from London to New York emits approximately \tcooe{1} per passenger~\cite{co2_flights}.
% Removed the other examples -- not sure it helps? 
     % \begin{enumerate}
...
The remainder of this report is structured as follows. Section~\ref{sec:background} provides background on emissions scopes and methodology. Section~\ref{sec:azure} covers emissions from Microsoft Azure, Section~\ref{sec:hpc} covers emissions from Baskerville HPC and Section~\ref{sec:github} covers emissions from GitHub Actions. Finally, we summarise our findings and outline potential areas for reducing emissions.
...
\begin{table}[ht]
  \centering
  \sisetup{table-align-text-after=false}
  \begin{tabular}{@{}llSS}\toprule
    & & \multicolumn{2}{c}{{Emissions ${} / \, \unit[qualifier-mode=combine]{\tonne\of{\cooe}}$}} \\
    \cmidrule(l){3-4}
    \multirow{-2}{*}{Approach} & \multirow{-2}{*}{Averaging method} & {All time} & {Year to date} \\ \midrule
    CHP plant      & Yearly & 89.2  & 25.0 \\
    National Grid  & Yearly & 36.9  & 10.3 \\
    National Grid  & Daily  & 48.5  & 10.9 \\
    National Grid  & Hourly & {---} & 11.2 \\ \bottomrule
  \end{tabular}
  \caption{Comparison of the different methods for calculating operational emissions from HPC.\label{tab:op_emissions_comparison}}
\end{table}
...
This report explores three different sources of computing emissions from the Turing: Microsoft Azure, Baskerville HPC and GitHub Actions. The Turing's emissions from these compute resources are summarised in Table~\ref{tab:summary_table}. In total, our use of these resources accounted for around \tcooe{34} emissions in the past year. This is equivalent to just under seven times the average UK per-capita emissions or around 35 passengers on a transatlantic flight to New York. More than anything, this demonstrates the huge emissions impact of flying.
...
An alternative approach, especially in light of Baskerville's end of life in 2026, would be to encourage Turing projects to move to greener HPCs such as those listed in the Green500~\cite{green500}. Other potential avenues for reducing the Turing's emissions from Baskerville usage, and which also apply to usage of other HPCs, include reducing the number of failed jobs --- for example, by providing more training or guidance for HPC users --- and improving the efficiency of jobs by optimising the compute resources requested or using more energy efficient algorithms.

Other sources of emissions not covered in this report due to lack of data include other UK HPCs which the Turing makes use of (Isambard-AI and Dawn), Turing-owned hardware (laptops, commercial GPUs and Pi's) and collaborators' hardware.
